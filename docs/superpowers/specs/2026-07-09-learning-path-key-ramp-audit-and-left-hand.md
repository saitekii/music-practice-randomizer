...
**Required mechanism change #1 — `applyStage()`:** its chord-checkbox handling (`ALL_CHORDS = CHORD_TYPES.map(c => c.id).concat(['inversions'])`) has no entry for `leftHandMode` at all today — no existing Learning Path stage has ever controlled that checkbox, since it didn't exist yet when `applyStage()` was written. `ALL_CHORDS` needs `'leftHandMode'` added, and each of the 5 new stages needs `'leftHandMode'` in its `chords` array (alongside `'chordMajor'`/`'chordMinor'` as appropriate) for `applyStage()` to check it. Every other existing stage's `chords` array is unaffected (none of them mention `leftHandMode`, so `applyStage()` will correctly uncheck it whenever navigating to any other stage — matching how it already unchecks every chord-type checkbox not listed).

**Required mechanism change #2 — `getStageMastery()`:** found while gathering implementation detail, not something the mastery system already handles correctly. `recordAdaptiveResult` deliberately isolates left-hand answers ([[left-hand-mode-deferred]]) — an LH answer updates *only* `adaptWeights.variations['Left Hand']`, never `adaptWeights.types`/`adaptWeights.roots`. But `getStageMastery()` doesn't know that: for a stage whose `chords` array includes `leftHandMode` alongside `chordMajor`/`chordMinor`, it would currently still push the ordinary `{dim:'types', key:'Major'}` and `{dim:'roots', key:'C'}`-style items (from the co-listed `chordMajor` entry and the stage's `notes`) — items whose weights are driven entirely by *earlier, one-handed* practice (Phase 2/6), not by anything practiced on this new stage. The stage would show as instantly "100% mastered" the moment a learner reaches it, without a single left-hand answer given. Fix: when a stage's `chords` includes `leftHandMode`, `getStageMastery()` pushes exactly one item (`{dim:'variations', key:'Left Hand'}`) and skips its normal per-chord-type and per-root item generation for that stage entirely — matching exactly how left-hand practice is actually tracked. No other stage's mastery calculation changes (this only branches on the new `leftHandMode` marker, which no existing stage has).
...
- New Left-Hand phase: `applyStage()` on each of the 5 new stages correctly checks/unchecks `leftHandMode`, `chordMajor`, `chordMinor`, and notes; confirm `inversions` is never simultaneously checked (matches the existing mutual-exclusion behavior). `getStageMastery()` on each of the 5 stages returns exactly one item (`variations['Left Hand']`) — not per-chord-type or per-root items — and a pre-existing non-left-hand stage's mastery calculation is unaffected (regression).
